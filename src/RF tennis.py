...
def create_baseline_model():
    ...
    # 计算基准预测概率
    df['baseline_prob'] = baseline_model.predict_proba(X)[:, 1]
    
    print(f"基准模型特征 ({len(technical_features)}个): {technical_features}")
    print(f"基准模型准确率: {baseline_model.score(X, y):.3f}")
    ...
    return baseline_model, technical_features, X

baseline_model, tech_features, X_baseline = create_baseline_model()

def simulate_match_baseline(num_simulations=200):
...
print("\n开始基准模型模拟...")
sim_results_baseline = simulate_match_baseline(num_simulations=200)
